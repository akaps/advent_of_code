...
assert is_op(mulr, sample_code, sample_reg_bef, sample_reg_aft)
assert is_op(addi, sample_code, sample_reg_bef, sample_reg_aft)
assert is_op(seti, sample_code, sample_reg_bef, sample_reg_aft)
...
assert is_op(addr, [-1, 0, 1, 2], [1, 3, 0, -1], [1, 3, 4, -1])
assert is_op(addi, [-1, 0, 8, 2], [3, -1, -1, -1], [3, -1, 11, -1])
assert is_op(mulr, [-1, 0, 1, 2], [3, 2, -1, -1], [3, 2, 6, -1])
assert is_op(muli, [-1, 3, 18, 0], [-1, -1, -1, 2], [36, -1, -1, 2])
assert is_op(banr, [-1, 2, 1, 0], [-1, 13, 60, -1], [12, 13, 60, -1])
assert is_op(bani, [-1, 1, 13, 0], [-1, 60, -1, -1], [12, 60, -1, -1])
assert is_op(borr, [-1, 2, 1, 0], [-1, 13, 60, -1], [61, 13, 60, -1])
assert is_op(bori, [-1, 1, 13, 0], [-1, 60, -1, -1], [61, 60, -1, -1])
assert is_op(setr, [-1, 3, -92, 2], [-1, -1, -1, 96], [-1, -1, 96, 96])
assert is_op(seti, [-1, 200, 10000, 3], [-1, -1, -1, -1], [-1, -1, -1, 200])

#comparison ops tests
assert is_op(gtir, [-1, 19, 0, 3], [-1, -1, -1, -1], [-1, -1, -1, 1])
assert is_op(gtir, [-1, -19, 0, 3], [-1, -1, -1, -1], [-1, -1, -1, 0])
assert is_op(gtri, [-1, 2, 2, 0], [-1, -3, 3, -1], [1, -3, 3, -1])
assert is_op(gtri, [-1, 1, 2, 0], [-1, -3, 3, -1], [0, -3, 3, -1])
assert is_op(gtrr, [-1, 1, 2, 3], [-1, 1, 2, -1], [-1, 1, 2, 0])
assert is_op(gtrr, [-1, 1, 2, 3], [-1, 2, 1, -1], [-1, 2, 1, 1])
assert is_op(eqri, [-1, 0, 19, 3], [19, -1, -1, -1], [19, -1, -1, 1])
assert is_op(eqri, [-1, 0, 19, 3], [20, -1, -1, -1], [20, -1, -1, 0])
assert is_op(eqrr, [-1, 1, 2, 3], [-1, 1, 1, -1], [-1, 1, 1, 1])
assert is_op(eqrr, [-1, 1, 2, 3], [-1, 420, 69, -1], [-1, 420, 69, 0])
