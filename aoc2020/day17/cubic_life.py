...
class CubicLife:
    def __init__(self, file_name):
        ...
        for x_index, row in enumerate(matrix):
            for y_index, val in enumerate(row):
                if val == '#':
                    self.cells[(x_index, y_index, z_index)] = True

    def count_adj(self, coordinate):
        count = 0
        adj = get_adj(coordinate)
        for coord in adj:
    ...
    def run_cycle(self):
        ...
        for cell_coordinate in to_process:
            ...
            neighbors = self.count_adj(cell_coordinate)
